...
class NCFTrainer:
    ...
    def train(
        ...
        Args:
            train_loader: Training data loader (with negative samples)
        ...
        Returns:
            Trained model
        """
        logger.info("Starting Neural CF training...")
        ...
        self.model = NeuralCF(
            n_users=n_users,
            n_items=n_items,
            gmf_embedding_dim=model_config['gmf_embedding_dim'],
            mlp_embedding_dim=model_config['mlp_embedding_dim'],
            mlp_hidden_layers=model_config['mlp_hidden_layers'],
            dropout=model_config['dropout'],
            use_pretrain=model_config.get('use_pretrain', False)
        ).to(self.device)
        ...
        # Gradient clipping
        self.max_grad_norm = self.config['training'].get('gradient_clip_max_norm', 5.0)
        ...
        logger.info("Neural CF training complete")
        return self.model
    ...
    def _train_epoch(self, train_loader) -> float:
        ...
        for user_indices, item_indices, labels in tqdm(train_loader, desc="Training", leave=False):
            # Move to device
    ...
    def _validate_epoch(self, val_loader) -> float:
        ...
        with torch.no_grad():
            for user_indices, item_indices, ratings in val_loader:
                ...
                # Convert ratings to binary labels (implicit feedback)
                labels = (ratings >= 4.0).float().to(self.device)
...
def train_neural_cf(
    config_path: str = "configs/ncf_config.yaml",
    dataset_dir: str = "dataset"
) -> NeuralCF:
    """
    Train Neural CF model from config file.
    ...
    train_loader, test_loader = data_manager.create_dataloaders(
        ...
        use_negatives=config['training']['use_negative_sampling'],
        shuffle=True
    )
